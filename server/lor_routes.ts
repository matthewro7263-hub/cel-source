...
export function registerLorRoutes(app: Express) {
  ...
  app.post("/api/projects/:id/lor_check_script", requireAuth, async (req, res) => {
    ...
    for (const noun of uniqueProperNouns) {
      ...
      const matchedFact = facts.find((f: LorContinuityFact) => f.title.toLowerCase().includes(noun.toLowerCase()));
      if (matchedFact) {
        if (!matchedEntities.find((e: LorContinuityFact) => e.id === matchedFact.id)) {
