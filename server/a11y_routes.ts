...
export function registerA11yRoutes(app: Express) {
  app.get("/api/a11y/prefs", attachOptionalUser, (req, res) => {
    ...
    let prefs = (storage as any).getA11yPrefs(authedUserId);
    if (!prefs) {
      prefs = (storage as any).createA11yPrefs({ userId: authedUserId, focusMode: 0, dyslexia: 0, colorblind: 0, reducedMotion: 0, largeTouch: 0, audioCues: 0 });
    }
    res.json(prefs);
  });

  app.post("/api/a11y/prefs", requireAuth, (req, res) => {
    ...
    const prefs = (storage as any).updateA11yPrefs(userId, body);
    res.json(prefs);
  });
}
