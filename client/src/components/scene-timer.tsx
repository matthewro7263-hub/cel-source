...
export function SceneTimerButton({
  sceneId,
  timerData,
  projectId,
}: {
  ...
  const { data: fetched } = useQuery<TimeData>({
  ...
  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["/api/scenes", sceneId, "time"] });
    if (projectId) {
      queryClient.invalidateQueries({ queryKey: ["/api/projects", projectId, "scene-timers"] });
    }
  };
...
export function SceneTimeBreakdown({
  sceneId,
  timerData,
}: {
  sceneId: number;
